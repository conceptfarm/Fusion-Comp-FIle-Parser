...
def lua_obj_parser(data):
	...
	return [lua_obj, data[2:]]

def lua_val_parser(data):
	# Matches simple values
	# "Something",
	# 123,
	res = lua_val_re.match(data.strip())
	...
	if res:
		...
		# return value , rest of the string
			...
			s = res.group(2)[:-1]
			return [ast.literal_eval(s), data[pos + 1:].strip()] #remove trailing comma
	else:
		return None

def lua_list_var_parser(data):
	res = lua_list_var_re.match(data.strip())
...
def lua_var_parser(data):
	...
	# SomeVarName = 123,
	res = lua_var_re.match(data.strip())

...
lua_list_re = re.compile( r'^([a-zA-Z0-9"_\-.\[\]]+)(?:\s=\s{)|^{' )
...
def lua_obj_parser(data, iterFile):
	# SomeVarName = Object {
	# Object {
	...
	data = feedData(data, res.end(), iterFile, [lua_obj, None])
	
	while data[0] != '}':
		res = lua_obj_parser(data, iterFile)
		if res:
			lua_obj.value.append(res[0])
			data = res[1]
		
		res = lua_list_parser(data, iterFile)
		if res:
			lua_obj.value.append(res[0])
			data = res[1]
		
		res = lua_var_parser(data, iterFile)
		if res:
			lua_obj.value.append(res[0])
			data = res[1]

		res = lua_list_var_parser(data, iterFile)
		if res:
			lua_obj.value.append(res[0])
			data = res[1]
		
		res = lua_val_parser(data, iterFile)
		if res:
			lua_obj.value.append(res[0])
			data = res[1]
			
		data = feedData(data, 0, iterFile, [lua_obj, None])
		#if data == "":
		#	try:
		#		data = next(iterFile).strip()
		#	except:
		#		return [lua_obj, None]
...
def feedData(data, slicePos, iterFile, exceptReturn = None):
	result = data[slicePos:].strip()
	if result == "":
		try:
			result = next(iterFile).strip()
		except:
			result = exceptReturn
	return result

def lua_val_parser(data, iterFile):
	...
	if res:
		data = feedData(data, res.end(), iterFile)
		if res.group(1):
			return [res.group(1)[:-1], data, iterFile] #remove trailing comma
		elif res.group(2) != None:
			s = res.group(2)[:-1]
			return [ast.literal_eval(s), data] #remove trailing comma
	else:
		return None

def lua_list_var_parser(data, iterFile):
	...
	pos = data.find('}')
	data = feedData(data, (pos + 2), iterFile)

	return [lua_list, data]

def lua_list_parser(data, iterFile):
	# SomeVarName = {
	# SomeVarName = { },
	# { #empty start of list
	...
	data = feedData(data, res.end(), iterFile, [lua_list, None])
	...
	while data[0:2] != "},":
		...
		data = feedData(data, 0, iterFile, [lua_list, None])
...
def lua_var_parser(data, iterFile):
	...
	data = feedData(data, res.end(), iterFile)
	return [LuaVariable(res.group(1), res.group(2)), data]
...
def all_parsers(*args):
	return lambda data, file: (reduce(lambda f, g: f if f(data, file) else g, args)(data, file))

def parse_comp_file(file_name):
	lua_value_parser = all_parsers(lua_val_parser, lua_var_parser,lua_obj_parser,lua_list_parser)
...
def parse_comp_file_v2(file_name):
	lua_value_parser = all_parsers(lua_obj_parser)

	res = []

	with open(file_name) as fp:
		iterFile =  iter(fp.readline, '')
		line = next(iterFile)
		output = lua_value_parser(line.strip(), iterFile)
		res = res + output
...
def main():
	file_name = "test.comp"
	comp = parse_comp_file_v2(file_name)
	if comp:
		Loaders = get_all_by_className(comp, "Loader")
		for o in Loaders:
			print(o)
	else:
